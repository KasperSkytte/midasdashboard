...
egaamarselisborg <- amp_subset_samples(amp_load(otutable = read.delim("data-raw/amplicon_data/aarhusvand CP332/otutable.txt", check.names = FALSE), metadata = openxlsx::read.xlsx("data-raw/amplicon_data/aarhusvand CP332/CP332_metadata.xlsx", rows = 1:21, cols = c(1,9,11))), normalise = TRUE)
...
usethis::use_data(cp332, overwrite = TRUE)
